...
const emptyBrand = (): Brand => ({
  name:'', tag:'NEW', logo:'', image:'', imagePos:'center',
  minDep:'', bonus:'', cashback:'', freeSpins:'', code:'', link:'',
  theme:{ accent:'#22c55e', shadow:'rgba(34,197,94,.45)' }, payments:['mbw','mb','visa','mc','btc'],
});
...
export default function ModeratorPage() {
  ...
  const addBrand = () => { setBrands(b => [...b, emptyBrand()]); setSelected(brands.length); };
  const removeBrand = (i:number) => setBrands(b => b.filter((_,idx)=>idx!==i));
  const move = (i:number, dir:-1|1) => setBrands(b => {
    const j = i + dir; if (j < 0 || j >= b.length) return b.slice();
    const arr = b.slice(); const [x] = arr.splice(i,1); arr.splice(j,0,x); return arr;
  });
        ...
        {brands.map((b, i) => (
          <div key={i} className="rounded-2xl p-4 bg-white/10 ring-1 ring-white/15 text-white">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <Input label="Payments (csv: mbw,mb,visa,mc,btc)"
                val={(b.payments||[]).join(',')}
